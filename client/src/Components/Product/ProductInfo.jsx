...
export default function ProductInfo() {
  const { token, data, cart, setCart, products, fetchProducts } =
    ...
    try {
      if (token) {
        let res = await axios.put(
          `${process.env.REACT_APP_ORIGIN}add/${data.userData._id}/${id}`
        );
        if (res.data.success) {
          setStock(res.data.stock);
          console.log(res.data);
          if (res.data.outOfStock) setMessage(res.data.message);
          else {
            setMessage("");
          }
          setCart(!cart);
  ...
  const removeFromCart = async () => {
    try {
      if (token) {
        let res = await axios.put(
          `${process.env.REACT_APP_ORIGIN}remove/${data.userData._id}/${id}`
        );
        if (res.data.success) {
          // setButton("Added")
          console.log(res.data);
  ...
  const getQuantity = () => {
    const index = data?.userData?.cart.findIndex(
      (p) => p.product._id == productData._id
    );
            ...
            {productData.images?.map((img, i) => {
              return (
                <img
                  src={img}
                  key={i}
                  className="shrink-0 w-full h-full  object-contain snap-center"
                  alt="product"
                ></img>
              );
            })}
          ...
          <div className="right w-10/12 md:w-1/2 h-full lg:px-12">
            <section className="flex flex-col gap-4 items-start relative">
              <div className="flex flex-col gap-3">
                <h1 className="title text-2xl md:text-4xl font-extrabold">
                  {productData.title}
                </h1>
                <p className="flex gap-2">
                  <span className="rating">⭐ {productData.rating}</span>
                  <span className="reviews border-l border-l-slate-400 pl-2">
                    1.2k reviews
                  </span>
                </p>
                <div className="price text-3xl">
                  <p>
                    ...
                    <span className="text-sm opacity-50 sora ml-2">
                      <span>M.R.P</span>
                ...
                {token ? (
                  <>
                    {getQuantity() !== null ? (
                      ...
                      <button
                        className={`text-md px-5 py-2 rounded-lg bg-[var(--secondary)] ${
                          data?.userData?.role === "admin" && "hidden"
                        }`}
              ...
              <p className="text-red-500">
                {stock < 10 && stock !== 0 && `Only ${stock} items left`}
              </p>
              <button
                className="absolute right-4 -top-6 bg-slate-300 w-8 h-8 rounded-full flex items-center justify-center"
                onClick={handleShare}
              >
